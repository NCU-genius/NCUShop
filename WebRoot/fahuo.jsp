...
if(a.equals("insert")){
    ...
    String charuid = new CommDAO().insert(request,"fahuo",ext); // 将数据提交到后台代码执行，查看src/dao/CommDAO.java 文件的insert 方法
new CommDAO().commOper("update shangpindingdan set dingdanzhuangtai='待签收' where id='"+request.getParameter("shangpindingdanid")+"'");
    ...
    String charuid = new CommDAO().update(request,"fahuo",ext); // 将数据提交到后台代码执行，查看src/dao/CommDAO.java 文件的insert 方法
    ...
    // 获取一行数据
    HashMap map = Query.make("fahuo").find(Request.get("id"));
    
    new CommDAO().delete(request,"fahuo");  // 执行删除命令
